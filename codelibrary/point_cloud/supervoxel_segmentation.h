...
#include "codelibrary/point_cloud/sampling/grid_sampling.h"
 ...
 * based method is performed, which is efficient and effective.
 *
 * Greater details please see the reference.
 *
 * Reference:
 *   Yangbin Lin, Cheng Wang, Dawei Zhai, Wei Li, Jonathan Li, Toward Better
 *   Boundary Preserved Supervoxel Segmentation for 3D Point Clouds. ISPRS
 *   Journal of Photogrammetry and Remote Sensing, 143, 39-47, 2018.
 *
 * Parameters:
 *  points        - the given point cloud.
 *  neighbors     - the neighbors for each point.
 *  n_supervoxels - the expected number of supervoxels.
 *  metric        - distance metric for points.
 *  supervoxels   - the indices of representation points.
 *  labels        - the index of supervoxel for each point.
 */
...
void SupervoxelSegmentation(const Array<Point>& points,
    ...
    // Minimization the energy function by fusion.
...
void SupervoxelSegmentation(const Array<Point>& points,
                            ...
                            Array<int>* labels) {
    Array<int> sampling;
    GridSampling sampler(resolution);
    sampler.Sample(points.begin(), points.end(), &sampling);
